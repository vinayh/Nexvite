...
describe("view_submission -> register + DM", () => {
	it("registers with the stored access token, DMs success with the looked-up Nexudus Id, and logs to the channel", async () => {
	...
	it("DMs a friendly failure with a generic contact when the auth seed is missing", async () => {
		let dm: any;
		mockUserInfo();
		mockSlack("chat.postMessage", (b) => (dm = b));

		const res = await run(await slackRequest("/slack/interactivity", submissionBody()), withoutAuth);
		expect(res.status).toBe(200);
		expect(dm.channel).toBe("U1");
	...
	it("refreshes on 401, persists the rotated pair, retries, and DMs success", async () => {
		...
		// Refresh used the stored refresh token + client_id = the account email.
		...
		// Retry used the new access token; the Durable Object keeps the rotated pair.
		expect(retried?.auth).toBe("Bearer new-access");
		expect(await testEnv.NEXUDUS_AUTH.getByName(testEnv.NEXUDUS_SUBDOMAIN).getAuth(testEnv.NEXUDUS_AUTH_SEED)).toEqual({
	...
	it("does not persist a malformed refresh response", async () => {
		let dm: any;
		mockUserInfo();
		mockVisitors(undefined, 401);
