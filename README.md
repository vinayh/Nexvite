...
Two optional fields turn the arrival into a series: **Repeat visit** (Every day / Every weekday / Every week / Every 2 weeks; defaults to "Does not repeat") and **Repeat until**, the inclusive last-possible visit date. The arrival fields define the first visit; every occurrence keeps the same space-local time, converted to UTC per occurrence so a series crossing a DST change stays at the same wall-clock. Nexudus has no recurrence field — the series is one `POST /api/public/visitors` with one visitor object per date, the pattern the Nexudus API documents for recurring visits — so a series is still a single Nexudus call. Series are capped at 30 visits (the Nexudus portal's own "up to 30 in one go" limit); longer series, a repeat with no end date, an end date before the first visit, or a weekday repeat starting on a weekend are bounced with inline errors. Each visit is an independent Nexudus record and the visitor receives a separate invite (own PIN/QR) per visit; the ✅ message says so and lists every Nexudus Id.
...
Every ✅ message's **Delete registration** button holds the Nexudus Id(s) captured at registration — a repeating series gets one **Delete all N registrations** button for the whole series (per-visit deletion stays in the portal). Clicking it (behind a confirm dialog) drives direct `DELETE /api/public/visitors/{id}` calls and replaces the message with a 🗑️ confirmation: the ✅ message restyled with the visitor's fields struck. Series deletes run in sequence, pausing between batches of 8 to stay under the public API's 10-requests-per-5s limit, so a full 30-visit series takes ~15s in the background. A `404`/`410` reports "may already be deleted" (for a series, visits that are already gone are noted while the rest still delete); the DM and channel copies of a registration are not kept in sync, so the other copy's button may already have been used. Anyone who can see a ✅ message can use its button. The submitter and the log channel's members are the same trusted group.
