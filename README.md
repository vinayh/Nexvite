...
| `SPACE_TIMEZONE` | IANA timezone of the space; the modal's arrival pickers are *read* in it and messages *display* arrival times in it (`ExpectedArrival` itself is sent as UTC). The modal labels hardcode "UK time", so keep it `Europe/London` or update the labels too |
